...
function Aiapp() {
  ...
  const parseFlashcards = (flashcardsString) => {
    ...
    try {
        ...
        const flashcardBlocks = flashcardsString.split(/\nFlashcard \d+:\n/g).filter(block => block.trim());
        ...
        return flashcards.length > 0 ? flashcards : [
            { question: "No flashcards could be parsed correctly", answer: "Please check the format of your data" }
        ];
    } catch (error) {
        console.error("Error parsing flashcards:", error);
        return [
            { question: "Error parsing flashcards", answer: "An error occurred while processing the flashcards data" }
        ];
    }
};
  ...
  const handleGenerateFlashcards = async () => {
    ...
    try {
      ...
      const flashcardsStr = typeof response.data === 'string' 
        ? response.data 
      ...
      if (parsedFlashcards.length > 0) {
        ...
        setFinalAnswer((item)=>{
          return item+parseFlashcards
        })
        console.log('happy'+finalAnswer)
  ...
  // Mock data for summary and Q&A
  const mockSummaryData = {
    summary: "StudyBuddy AI uses a Layered Architecture consisting of Presentation, Application, Data Access, and Integration layers. Key components include DocumentManager for uploads, SummaryGenerator for document processing, and various tracking tools like StudySessionManager. The system is hosted on AWS with S3/CloudFront for frontend, EC2 for backend, RDS for MySQL database, and Lambda for AI processing. Security measures include firewalls, encryption, and Privy.io for authentication."
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-100">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-10">
          <h1 className="text-4xl font-bold text-indigo-800 mb-2">StudyBuddy AI</h1>
          <p className="text-gray-600">Upload documents to generate summaries, flashcards, and practice questions</p>
        </div>
              ...
              >
                ...
                <div className="flex flex-col items-center justify-center text-center">
                  ...
                  {fileName ? (
                    ...
                    <>
                      <p className="text-indigo-700 font-medium mb-1">Click to upload a document</p>
        ...
        {isProcessing && (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-indigo-800 font-medium">Processing your document...</p>
            </div>
          </div>
        )}
        ...
        {!isProcessing && (
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            {currentTab === 'summary' && (
              <div>
                <h2 className="text-2xl font-semibold text-indigo-800 mb-4">Document Summary</h2>
                <div className="bg-indigo-50 p-6 rounded-lg border border-indigo-100">
                  <p className="text-gray-700 leading-relaxed">{displaySummary.summary}</p>
                </div>
              </div>
            )}

            {/* Flashcards Tab */}
            {currentTab === 'flashcards' && flashcardsData && flashcardsData.length > 0 && (
              <div>
                <h2 className="text-2xl font-semibold text-indigo-800 mb-4">Flashcards</h2>
            ...
            {currentTab === 'flashcards' && (!flashcardsData || flashcardsData.length === 0) && (
              <div className="text-center py-8">
                <div className="text-indigo-800 font-medium">No flashcards available</div>
                <p className="text-gray-600 mt-2">Click the Flashcards button to generate flashcards from your document</p>
              </div>
            )}
